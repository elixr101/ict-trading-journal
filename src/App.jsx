...
// ─── TRACKED INPUT WRAPPERS (For Trade Log Validation) ───────────────────────
function TrackedInput(props) {
  const [t, setT] = useState(false);
  const val = props.type === "number" && (props.value === 0 || props.value === "0") ? "" : props.value;
  const empty = val === "" || val === null || val === undefined;
  const b = t ? (empty ? "1px solid #f87171" : "1px solid #4ade80") : undefined;
  return <Input {...props} onBlur={(e) => { setT(true); if (props.onBlur) props.onBlur(e); }} style={{ ...(b ? { border: b } : {}), ...(props.style || {}) }} />;
}
function TrackedSelect(props) {
  const [t, setT] = useState(false);
  const empty = props.value === "" || props.value === null || props.value === undefined;
  const b = t ? (empty ? "1px solid #f87171" : "1px solid #4ade80") : undefined;
  return <Select {...props} onBlur={(e) => { setT(true); if (props.onBlur) props.onBlur(e); }} style={{ ...(b ? { border: b } : {}), ...(props.style || {}) }} />;
}
function TrackedTextArea(props) {
  const [t, setT] = useState(false);
  const empty = props.value === "" || props.value === null || props.value === undefined;
  const b = t ? (empty ? "1px solid #f87171" : "1px solid #4ade80") : undefined;
  return <TextArea {...props} onBlur={(e) => { setT(true); if (props.onBlur) props.onBlur(e); }} style={{ ...(b ? { border: b } : {}), ...(props.style || {}) }} />;
}

// ─── TRADE FORM ───────────────────────────────────────────────────────────────
function TradeForm({trade,accounts,customModels,customConcepts,onSave,onCancel,onAddModel,onAddConcept,defaultSession,defaultInstrument}){
  const {isMobile} = useIsMobile();
  const [form,setForm]=useState(trade||{id:"",date:new Date().toISOString().slice(0,10),time:"",exitTime:"",instrument:defaultInstrument||"NQ",session:defaultSession||"NY AM",direction:"Long",contracts:1,entry:"",stop:"",target:"",exit:"",pnl:0,ictConcepts:[],emotions:["Calm"],grade:"B",accountId:"",notes:"",rr:"",partials:"",preTradeNarrative:"",postTradeReview:"",htfBias:"",ltfEntry:"",confluenceScore:0,mistakes:[],rulesFollowed:[],drawOnLiquidity:"",entryModel:"",chartUrl:""});
  const [nc,setNC]=useState("");
  const [tModel,setTModel]=useState(false);
  const [tConcept,setTConcept]=useState(false);
  const set=(k,v)=>setForm(f=>({...f,[k]:v}));
  const allC=[...ICT_CONCEPTS,...(customConcepts||[])];
  const allM=[...DEFAULT_MODELS,...(customModels||[])];
  useEffect(()=>{if(form.emotion&&!form.emotions)setForm(f=>({...f,emotions:[f.emotion]}));},[]);
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 1fr":"1fr 1fr 1fr 1fr 1fr",gap:10}}>
        ...
        <TrackedSelect label="Session (Auto)" value={form.session} onChange={e=>set("session",e.target.value)} options={SESSIONS} disabled={true} style={{opacity: 0.5, pointerEvents: "none", backgroundColor: "rgba(0,0,0,0.2)"}}/>
      </div>
      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 1fr":"1fr 1fr 1fr 1fr 1fr",gap:10}}>
        <div style={{gridColumn:isMobile?"span 2":"auto"}}>
          <TrackedSelect label="Direction" value={form.direction} onChange={e=>set("direction",e.target.value)} options={["Long","Short"]}/>
        </div>
        <TrackedInput label="Contracts" type="number" value={form.contracts} onChange={e=>set("contracts",+e.target.value)}/>
        <TrackedInput label="Entry" type="number" step="any" value={form.entry} onChange={e=>set("entry",e.target.value)}/>
        <TrackedInput label="Stop" type="number" step="any" value={form.stop} onChange={e=>set("stop",e.target.value)}/>
        <TrackedInput label="Target" type="number" step="any" value={form.target} onChange={e=>set("target",e.target.value)}/>
      </div>
      <div style={{display:"grid",gridTemplateColumns:g,gap:10}}>
        <TrackedInput label="Exit" type="number" step="any" value={form.exit} onChange={e=>set("exit",e.target.value)}/>
        <TrackedInput label="P&L (auto)" type="number" step="any" value={form.pnl} onChange={e=>set("pnl",+e.target.value)}/>
        <TrackedInput label="R:R (auto)" value={form.rr} onChange={e=>set("rr",e.target.value)}/>
        <TrackedSelect label="Grade" value={form.grade} onChange={e=>set("grade",e.target.value)} options={TRADE_GRADES}/>
      </div>
      <TrackedSelect label="Account" value={form.accountId} onChange={e=>set("accountId",e.target.value)} options={[{value:"",label:"-- No Account --"},...accounts.map(a=>({value:a.id,label:`${a.name} (${a.firm} $${a.currentBalance.toLocaleString()})`}))]}/>
      <TrackedInput label="Chart URL" value={form.chartUrl||""} onChange={e=>set("chartUrl",e.target.value)} placeholder="https://tradingview.com/x/..."/>
      
      {sec("Pre-Trade Rules")}
      <div style={{display:"flex",flexWrap:"wrap",gap:3,justifyContent:"center"}}>{RULE_CHECKLIST.map(r=><Pill key={r} label={r} active={form.rulesFollowed.includes(r)} onClick={()=>toggle("rulesFollowed",r)} color="rgba(34,197,94,0.18)"/>)}</div>
      
      {sec("Psychology")}
      <div><label style={{...ulbl,display:"block",marginBottom:4,textAlign:"center"}}>Emotions <span style={{fontWeight:400,textTransform:"none",color:"rgba(255,255,255,0.2)"}}>(multi)</span></label><div style={{display:"flex",flexWrap:"wrap",justifyContent:"center"}}>{EMOTIONS.map(e=><Pill key={e} label={e} active={ems.includes(e)} onClick={()=>toggleEmo(e)} color={NEGATIVE_EMOTIONS.includes(e)?"rgba(239,68,68,0.18)":"rgba(34,197,94,0.18)"}/>)}</div></div>
      <div><label style={{...ulbl,display:"block",marginBottom:6,textAlign:"center"}}>Mistakes</label><div style={{display:"flex",flexWrap:"wrap",justifyContent:"center"}}>{MISTAKES.map(m=><Pill key={m} label={m} active={form.mistakes.includes(m)} onClick={()=>toggle("mistakes",m)} color="rgba(239,68,68,0.18)"/>)}</div></div>
      
      {sec("Bias & Model")}
      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr 1fr",gap:10}}>
        <TrackedSelect label="HTF Bias" value={form.htfBias} onChange={e=>set("htfBias",e.target.value)} options={["","Bullish","Bearish","Neutral/Ranging"]}/>
        <TrackedSelect label="LTF TF" value={form.ltfEntry} onChange={e=>set("ltfEntry",e.target.value)} options={["",...TIMEFRAMES]}/>
        <TrackedInput label="Draw on Liquidity" value={form.drawOnLiquidity} onChange={e=>set("drawOnLiquidity",e.target.value)} placeholder="BSL above PDH"/>
      </div>
      <div>
        <label style={{...ulbl,display:"block",marginBottom:6,textAlign:"center"}}>Entry Model</label>
        <div style={{display:"flex",flexWrap:"wrap",marginBottom:6,justifyContent:"center"}}>{allM.map(m=><Pill key={m} label={m} active={form.entryModel===m} onClick={()=>set("entryModel",m)} color="rgba(56,189,248,0.18)"/>)}</div>
        <div style={{display:"flex",gap:6}}>
          <input onBlur={()=>setTModel(true)} value={form.entryModel} onChange={e=>set("entryModel",e.target.value)} placeholder="Or type custom..." style={{flex:1,background:"rgba(255,255,255,0.04)",border:tModel?(form.entryModel?"1px solid #4ade80":"1px solid #f87171"):"1px solid rgba(255,255,255,0.08)",borderRadius:7,padding:"8px 11px",color:"#e2e8f0",fontSize:12,fontFamily:"'DM Mono',monospace",outline:"none",boxSizing:"border-box",transition:"all 0.2s"}}/>
          {form.entryModel&&!allM.includes(form.entryModel)&&<Btn variant="success" style={{padding:"7px 12px",fontSize:10}} onClick={()=>onAddModel(form.entryModel.trim())}>+ Save</Btn>}
        </div>
      </div>
      
      {sec("ICT Concepts")}
      <div style={{display:"flex",flexDirection:"column",gap:14}}>{Object.entries(CATEGORIZED_ICT_CONCEPTS).map(([cat,concepts])=><div key={cat}><div style={{fontSize:10,color:"rgba(255,255,255,0.4)",textTransform:"uppercase",textAlign:"center",marginBottom:6,fontWeight:700}}>{cat}</div><div style={{display:"flex",flexWrap:"wrap",justifyContent:"center"}}>{concepts.map(c=><Pill key={c} label={c} active={form.ictConcepts.includes(c)} onClick={()=>toggle("ictConcepts",c)} color="rgba(168,85,247,0.18)"/>)}</div></div>)}{customConcepts&&customConcepts.length>0&&(<div><div style={{fontSize:10,color:"rgba(255,255,255,0.4)",textTransform:"uppercase",textAlign:"center",marginBottom:6,fontWeight:700}}>CUSTOM</div><div style={{display:"flex",flexWrap:"wrap",justifyContent:"center"}}>{customConcepts.map(c=><Pill key={c} label={c} active={form.ictConcepts.includes(c)} onClick={()=>toggle("ictConcepts",c)} color="rgba(168,85,247,0.18)"/>)}</div></div>)}</div>
      <div style={{display:"flex",gap:6}}>
        <input onBlur={()=>setTConcept(true)} value={nc} onChange={e=>setNC(e.target.value)} placeholder="Add custom concept..." onKeyDown={e=>{if(e.key==="Enter"&&nc.trim()&&!allC.includes(nc.trim())){onAddConcept(nc.trim());toggle("ictConcepts",nc.trim());setNC("");}}} style={{flex:1,background:"rgba(255,255,255,0.04)",border:tConcept?(nc?"1px solid #4ade80":"1px solid #f87171"):"1px solid rgba(255,255,255,0.08)",borderRadius:7,padding:"8px 11px",color:"#e2e8f0",fontSize:12,fontFamily:"'DM Mono',monospace",outline:"none",boxSizing:"border-box",transition:"all 0.2s"}}/>
        {nc.trim()&&!allC.includes(nc.trim())&&<Btn variant="success" style={{padding:"7px 12px",fontSize:10}} onClick={()=>{onAddConcept(nc.trim());toggle("ictConcepts",nc.trim());setNC("");}}>+ Save</Btn>}
      </div>
      ...
      <div style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",background:"rgba(255,255,255,0.025)",borderRadius:8,border:"1px solid rgba(255,255,255,0.05)"}}>
        <span style={ulbl}>Confluence</span>
        <div style={{display:"flex",flex:1,gap:3}}>{Array.from({length:10}).map((_,i)=>{const fill=Math.min(1,Math.max(0,form.confluenceScore-i));const bc=form.confluenceScore>=7?"#4ade80":form.confluenceScore>=4?"#fbbf24":"#f87171";return<div key={i} style={{flex:1,height:8,borderRadius:2,background:fill>=1?bc:fill>0?`linear-gradient(90deg,${bc} ${fill*100}%,rgba(255,255,255,0.06) ${fill*100}%)`:"rgba(255,255,255,0.06)"}}/>;})}</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:10}}>
        <TrackedTextArea label="Pre-Trade" value={form.preTradeNarrative} onChange={e=>set("preTradeNarrative",e.target.value)} placeholder="HTF bias? DOL?"/>
        <TrackedTextArea label="Post-Trade" value={form.postTradeReview} onChange={e=>set("postTradeReview",e.target.value)} placeholder="What happened?"/>
      </div>
      <TrackedTextArea label="Partials" value={form.partials} onChange={e=>set("partials",e.target.value)} placeholder="TP1 at 1:1..."/>
      <TrackedTextArea label="Notes" value={form.notes} onChange={e=>set("notes",e.target.value)}/>
      ...
      <div style={{ width: "100%", maxWidth: 400, boxSizing: "border-box" }}>
        ...
        <div style={{ padding: 28, boxSizing: "border-box" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            {err && <div style={{ fontSize: 12, color: err.includes("Check") ? "#4ade80" : "#f87171", padding: "8px 12px", background: err.includes("Check") ? "rgba(34,197,94,0.08)" : "rgba(239,68,68,0.08)", borderRadius: 6, boxSizing: "border-box" }}>{err}</div>}
            <Btn variant="primary" onClick={go} style={{ width: "100%", marginTop: 4, opacity: loading ? 0.6 : 1, boxSizing: "border-box" }}>{loading ? "..." : (mode === "login" ? "Log In" : "Create Account")}</Btn>
          </div>
        </div>
      </div>
    </div>
